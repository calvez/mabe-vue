  ...
  <div>
    <section>
      <p>
        ...
        WordPress REST API endpoint. Take what you see here &amp; rip it apart
        to suit your needs. To improve it for everyone else,
        <a href="https://www.github.com/alexmacarthur/wp-vue"
  ...
  methods: {
    getPosts: async function() {
      ...
      try {
        response = await this.get(
          `/mabe_tagok?per_page=${POSTS_PER_PAGE}&page=${this.page}`
        );
    ...
    getAdjacentPageData: async function(prevPage = false) {
      ...
      if (page > 0) {
        try {
          response = await this.get(
            `/mabe_tagok?per_page=${POSTS_PER_PAGE}&page=${page}`
          );
        } catch (error) {
          console.error(error);
        }
